...
impl ProcessMonitor {
    ...
    pub fn start_monitoring(&mut self) -> Receiver<ProcessEvent> {
        ...
        self.known_pids = self.memory_scanner.system.processes().keys().cloned().collect();
            ...
            while !stop_signal.load(std::sync::atomic::Ordering::Relaxed) {
                ...
                for &pid in current_pids.difference(&known_pids) {
                    if let Some(process) = system.process(pid) {
                        ...
                        Self::scan_new_process(pid, process, &memory_scanner, &engine, &tx);
                    }
                }

                // 定期内存扫描 - 使用 memory_scanner
                Self::perform_periodic_memory_scan(&memory_scanner, &engine, &tx);
        ...
        for result in scan_results {
            if result.is_malicious {
                let event = ProcessEvent {
                    ...
                    event_type: ProcessEventType::SuspiciousActivity(result.reason),
                };
                let _ = tx.send(event);
