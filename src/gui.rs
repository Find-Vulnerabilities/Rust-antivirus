...
impl AntivirusApp {
    pub fn new(engine: Arc<AntivirusEngine>) -> Self {
        ...
        Self {
            ...
            selected_directory: dirs::home_dir()
        ...
        ui.vertical(|ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("Browse...").clicked() {
                    // 在实际应用中，这里会打开文件对话框
                    // 简化实现：使用默认目录
                    self.selected_directory = dirs::home_dir()
                        .unwrap_or_else(|| PathBuf::from("."))
                        .to_string_lossy()
                        .to_string();
                }
    ...
    fn start_scan(&mut self) {
        ...
        thread::spawn(move || {
            ...
            let results = engine.scan_directory(PathBuf::from(&directory).as_path(), deep_scan);
    ...
    fn start_cleanup(&mut self) {
        ...
        thread::spawn(move || {
            let (cleaned_files, freed_space) = engine.clean_junk_files(clean_temp, clean_zero_byte, clean_recycle_bin);
    ...
    fn start_memory_scan(&mut self) {
        ...
        thread::spawn(move || {
            let results = engine.scan_all_process_memory();
