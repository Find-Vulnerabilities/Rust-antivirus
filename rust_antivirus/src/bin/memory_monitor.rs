        ...
        for (pid, process) in system.processes() {
            ...
            if !known_pids.contains(&pid_value) {
                ...
                if let Some(exe_path) = process.exe() {
                    ...
                    // 执行启发式分析
                    if let Some((threat_name, risk_score)) = heuristic_scan_process(pid_value, &exe_str, &proc_name) {
                        threat_count += 1;
                        let now = Local::now().format("%H:%M:%S");
                        ...
                        // TODO: 通过IPC发送威胁告警到主程序
                        // send_threat_alert(pid_value, exe_str, threat_name, risk_score);
                    }
                }
