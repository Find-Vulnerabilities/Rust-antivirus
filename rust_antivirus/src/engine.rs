    ...
    fn protect_quarantined_path(path: &Path) -> bool {
        ...
        #[cfg(windows)]
        {
            ...
            if let Ok(entries) = std::fs::read_dir(path) {
                use winapi::um::fileapi::SetFileAttributesW as set_attrs;
                for entry in entries.flatten() {
                    let p = entry.path();
                    let wide_child: Vec<u16> = OsStr::new(&p)
                        .encode_wide()
                        .chain(std::iter::once(0))
                        .collect();
                    unsafe {
                        // try to apply same attributes to child entries; ignore errors
                        let _ = set_attrs(wide_child.as_ptr(), attrs);
                    }
                }
            }
        }
