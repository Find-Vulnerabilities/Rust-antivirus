...
// Add platform-specific imports for quarantine protection
#[cfg(unix)]
use std::os::unix::fs::PermissionsExt;

// Add Windows-only imports required for SetFileAttributesW and wide string conversion
#[cfg(windows)]
use std::os::windows::ffi::OsStrExt;
#[cfg(windows)]
use winapi::um::fileapi::SetFileAttributesW;
#[cfg(windows)]
use winapi::um::winnt::{FILE_ATTRIBUTE_HIDDEN, FILE_ATTRIBUTE_SYSTEM, FILE_ATTRIBUTE_READONLY};
...
pub struct AntivirusEngine {
    ...
    pub yara_scanner: Arc<crate::yara_engine::YaraEngine>,
    pub hash_cache: Arc<Mutex<HashMap<String, ScanResult>>>,
}

// 白名单哈希值
const WHITELISTED_HASHES: [&str; 24] = [
    ...
    "009dbc256c24a40ef642349076fffc30210eb9b9bdfa4d719bfcf185503298c7",
    "a04204944e66395492dff6f370f338cc9461e0b0e5149c0a8e9b885739833295",
    "efacbbadfc4a32e94499457ea170e64a696b7e639ef894f0ca55de89736ae543",
    "70ad6fa681d6878e717e31cb58668c7b8936f74775ec328dee86b07f83a54545",
    "6f69a14820e750b22662dc9339bfd22b9cfe51877c4103b6863f48efe6db956e",
...
impl AntivirusEngine {
    pub fn new() -> Result<Self, EngineError> {
        ...
        // Protect the quarantine directory (best-effort): set attributes/permissions so files inside
        // cannot be easily executed and folder is hidden/readonly (Windows) or owner-only (Unix).
        if !Self::protect_quarantined_path(&quarantine_dir) {
            log::warn!("Failed to fully protect the quarantine directory: {:?}", quarantine_dir);
        }
        ...
        let yara_scanner = match rules_path.to_str() {
            Some(rules_path_str) => {
                match crate::yara_engine::YaraEngine::with_rules(rules_path_str) {
                    Ok(e) => Arc::new(e),
                    Err(e) => {
                        log::warn!("Failed to load YARA rules from {}: {}", rules_path_str, e);
                        return Err(EngineError::Yara(e.to_string()));
                    }
                }
            }
            None => {
                return Err(EngineError::Yara("Invalid YARA rules path".to_string()));
            }
        };
    ...
    pub fn scan_file(&self, file_path: &Path) -> ScanResult {
        // NOTE: stop signal should NOT be reset per-file. Resetting should happen
        // once when starting a new overall scan (e.g., in scan_directory or GUI).
        // This preserves the ability to call stop() to interrupt an ongoing scan.
        ...
        let metadata = match std::fs::metadata(file_path) {
            ...
            Err(_) => return ScanResult {
                ...
                result: "Cannot read file metadata".to_string(),
                risk_score: 0,
        ...
        // 检查哈希缓存
        if let Ok(hash) = self.calculate_sha256(file_path) {
            let cache_key = format!("{}:{}:{}", hash, file_size, mtime);
            if let Some(cached) = self.hash_cache.lock().unwrap_or_else(|e| e.into_inner()).get(&cache_key) {
                log::info!("Cache hit for file: {:?}", file_path);
                return cached.clone();
            }
        }
        ...
        // 更新扫描计数
        {
            let mut count = self.scan_count.lock().unwrap_or_else(|e| e.into_inner());
            *count += 1;
        }

        // 检查停止信号
        if *self.stop_signal.lock().unwrap_or_else(|e| e.into_inner()) {
            return ScanResult {
                file_path: file_path.to_path_buf(),
        ...
        if self.is_whitelisted(file_path) {
            ...
            if let Ok(hash) = self.calculate_sha256(file_path) {
                let cache_key = format!("{}:{}:{}", hash, file_size, mtime);
                self.hash_cache.lock().unwrap_or_else(|e| e.into_inner()).insert(cache_key, result.clone());
            }
            return result;
        }
        ...
        if sandbox_matched {
            let mut threats = self.threats_found.lock().unwrap_or_else(|e| e.into_inner());
            *threats += 1;
        ...
        if let Ok(hash) = self.calculate_sha256(file_path) {
            let cache_key = format!("{}:{}:{}", hash, file_size, mtime);
            self.hash_cache.lock().unwrap_or_else(|e| e.into_inner()).insert(cache_key, scan_result.clone());
        }
    ...
    fn yara_scan_file(&self, file_path: &Path) -> Vec<String> {
        // Convert file path to String for API call
        let file_path_str = match file_path.to_str() {
            Some(p) => p,
            None => {
                log::warn!("Invalid file path: {:?}", file_path);
                return Vec::new();
            }
        };

        // Scan file with embedded libyara
        match self.yara_scanner.scan_file(file_path_str) {
            Ok(scan_result) => {
                // scan_result contains matched rules
    ...
    pub fn quarantine_file(&self, file_path: &Path, reason: &str) -> bool {
        ...
        // 生成唯一的隔离文件名，并附加不可执行后缀以避免运行
        let file_name = file_path.file_name().unwrap_or_default();
        let timestamp = chrono::Local::now().format("%Y%m%d_%H%M%S");
        // Add explicit ".quarantine" suffix so executables lose their .exe extension
        let quarantine_filename = format!("{}_{}.quarantine", timestamp, file_name.to_string_lossy());
        ...
        // Apply strict protections to the quarantined file (platform-specific)
        if !Self::protect_quarantined_path(&dest_path) {
            log::warn!("Failed to fully protect quarantined file: {:?}", dest_path);
            // continue even if protection failed; record still created
        }
    ...
    // Add helper to protect quarantined files (Unix: chmod 0o600 for files / 0o700 for dir, Windows: Hidden + ReadOnly + System)
    fn protect_quarantined_path(path: &Path) -> bool {
        // Try to set readonly flag as a generic first step and apply platform-specific protections.
        if let Ok(metadata) = fs::metadata(path) {
            // Determine if path is directory
            // Rename to _is_dir to silence unused variable warning
            let _is_dir = metadata.is_dir();
            ...
            #[cfg(unix)]
            {
                ...
                // Directories: owner read/write/execute (0o700) to prevent other users from listing.
                // Files: owner read/write (0o600) to prevent execution.
                let mode = if _is_dir { 0o700 } else { 0o600 };
                let permissions = fs::Permissions::from_mode(mode);
                if let Err(e) = fs::set_permissions(path, permissions) {
                    log::warn!("Failed to chmod quarantined path {:?}: {}", path, e);
                }

                // For directories, walk children and ensure files are non-executable (best-effort).
                if _is_dir {
                            ...
                            if let Ok(meta) = fs::metadata(&p) {
                                if meta.is_file() {
                                    ...
                                    if let Err(e) = fs::set_permissions(&p, perms) {
                                        log::warn!("Failed to chmod quarantined child file {:?}: {}", p, e);
                                    }
                                } else if meta.is_dir() {
                                    let dir_mode = 0o700;
                                    let perms = fs::Permissions::from_mode(dir_mode);
                                    if let Err(e) = fs::set_permissions(&p, perms) {
                                        log::warn!("Failed to chmod quarantined child dir {:?}: {}", p, e);
                                    }
                                }
                            }
                        }
            ...
            // Set readonly flag for all platforms (best-effort)
            if let Ok(mut perms) = fs::metadata(path).map(|m| m.permissions()) {
                perms.set_readonly(true);
                if let Err(e) = fs::set_permissions(path, perms) {
                    log::warn!("Failed to set readonly on quarantined path {:?}: {}", path, e);
                }
            } else {
                log::warn!("Could not retrieve metadata for quarantined path {:?}", path);
            }
        } else {
            log::warn!("Could not retrieve metadata for quarantined path {:?}", path);
        }

        // On Windows additionally set Hidden + System + ReadOnly attributes for stronger UI-level protection
        #[cfg(windows)]
        {
            // Combine attributes: Hidden + System + ReadOnly
            let attrs = (FILE_ATTRIBUTE_HIDDEN | FILE_ATTRIBUTE_SYSTEM | FILE_ATTRIBUTE_READONLY) as u32;

            unsafe {
                // use Path::as_os_str() and OsStrExt::encode_wide()
                ...
                if SetFileAttributesW(wide.as_ptr(), attrs) == 0 {
                    log::warn!("SetFileAttributesW failed for {:?}.", path);
                    // continue; attributes are best-effort
                } else {
                    log::info!("Applied Hidden/System/ReadOnly attributes to quarantine path {:?}", path);
                }
            }

            // Best-effort: for directories, attempt to mark children files similarly (helps hide/deny execution UI-level)
            if let Ok(entries) = std::fs::read_dir(path) {
                for entry in entries.flatten() {
                    let p = entry.path();
                    let wide_child: Vec<u16> = p.as_os_str()
                        .encode_wide()
                        .chain(std::iter::once(0))
                        .collect();
                    unsafe {
                        // call the actual API function directly
                        let _ = SetFileAttributesW(wide_child.as_ptr(), attrs);
                    }
                }
            }
        }
    ...
    fn get_suspicious_memory_regions(&self, pid: Pid) -> usize {
        ...
        // 基础启发式分析：检查进程是否被注入恶意代码的迹象
        let system = System::new_all();
        if let Some(process) = system.process(pid) {
            let proc_name = process.name().to_string_lossy().to_lowercase();
            
            // 可疑的内存特征
            let memory_ratio = process.memory() as f64 / 1024.0 / 1024.0; // MB
            
            // 异常内存占用（某些恶意软件会异常占用内存）
            if memory_ratio > 500.0 {
                suspicious_count += 1;
            }
            
            // 检查是否是已知的可疑进程名
            let suspicious_names = [
                "csrss.exe", "lsass.exe", "rundll32.exe", "wscript.exe", "cscript.exe",
                "powershell.exe", "cmd.exe"
            ];
            
            if suspicious_names.iter().any(|&name| proc_name.ends_with(name)) {
                // 这些进程需要进一步分析
                suspicious_count += 1;
            }
            
            // 检查虚拟内存大小（跨平台）
            if process.virtual_memory() > 2 * 1024 * 1024 * 1024 {  // 2GB
                suspicious_count += 1;
            }
        }
    ...
    fn save_deletion_list(&self) -> Result<(), std::io::Error> {
        ...
        let json_data = serde_json::to_string_pretty(&*deleted_files)
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e.to_string()))?;
        
        let log_path = utils::get_deletion_log_path();
        ...
        log::debug!("Deletion list saved to: {:?}", log_path);
    ...
    fn clean_recycle_bin(&self) -> (u32, u64) {
        ...
        #[cfg(windows)]
        {
            ...
            let recycle_paths = vec![
                ...
                PathBuf::from("D:\\$Recycle.Bin"),
                PathBuf::from("E:\\$Recycle.Bin"),
            ];
                                ...
                                if let Ok(dir_entries) = fs::read_dir(&path) {
                                    for dir_entry in dir_entries.flatten() {
                                        ...
                                        if let Ok(file_meta) = fs::metadata(&file_path) {
                                            if let Err(e) = fs::remove_file(&file_path) {
                                                log::warn!("Failed to delete recycle bin file {:?}: {}", file_path, e);
                                            } else {
                                                cleaned += 1;
                                                freed += file_meta.len();
                                            }
            ...
            log::info!("Cleaned {} files from recycle bin, freed {} bytes", cleaned, freed);
            (cleaned, freed)
        }
        
        #[cfg(not(windows))]
        {
            // 在非 Windows 系统上不进行回收站清理
            log::warn!("Recycle bin cleaning is Windows-only.");
            (0, 0)
        }
    }
