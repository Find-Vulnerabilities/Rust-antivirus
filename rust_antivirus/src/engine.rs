...
impl AntivirusEngine {
    ...
    pub fn scan_file(&self, file_path: &Path) -> ScanResult {
        ...
        if let Ok(metadata) = std::fs::metadata(file_path) {
            ...
            // TODO: 集成哈希缓存 - 在这里检查缓存
            // let cache = HashCache::new(Default::default());
            // if let Some(cached) = cache.get(&file_path.to_path_buf(), file_size, mtime) {
            //     return ScanResult { ... cached result ... };
            // }
        }
    ...
    fn yara_scan_file(&self, file_path: &Path) -> Vec<String> {
        ...
        // Use utils helper which prefers compiled "anti.yarac" if present, otherwise "anti.yar".
        let rules_path = utils::get_yara_rules_path();
        if !rules_path.exists() {
            log::warn!("No YARA rules found at expected path: {:?}", rules_path);
            return Vec::new();
        }

        // Determine if rules file is a compiled rules file (.yarac)
        let is_compiled = rules_path
            .extension()
            .and_then(|s| s.to_str())
            .map(|ext| ext.eq_ignore_ascii_case("yarac"))
            .unwrap_or(false);

        // Build command conditionally: use -C for compiled rules, otherwise use the prior flags.
        let mut cmd = Command::new(yara_exe);
        if is_compiled {
            cmd.arg("-C").arg(rules_path.as_os_str());
        } else {
