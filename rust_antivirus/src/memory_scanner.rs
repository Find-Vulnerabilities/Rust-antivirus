...
impl MemoryScanner {
    ...
    /// 读取指定内存区域的原始字节（受限大小以避免一次性读取过大数据）
    pub fn read_memory_region(&self, pid: Pid, address: usize, size: usize) -> Result<Vec<u8>> {
        // safety limits: cap to 1MB per read (adjust as needed)
        let max_read = 1024 * 1024;
        let read_size = if size == 0 { 0 } else { std::cmp::min(size, max_read) };
        if read_size == 0 {
            return Ok(Vec::new());
        }
        ...
        unsafe {
            ...
            if success == 0 {
                return Err(anyhow!("ReadProcessMemory failed for PID {} at {:x}", pid, address));
            }
