...
impl RealtimeMonitor {
    pub fn new(engine: Arc<AntivirusEngine>) -> Self {
        ...
        // Build monitored paths list per requirements
        let mut monitored_paths: Vec<PathBuf> = Vec::new();
        ...
        if let Ok(users_dir) = std::fs::read_dir(r"C:\Users") {
            for entry in users_dir.flatten() {
                ...
                if user_path.is_dir() {
                    let appdata = user_path.join("AppData");
                    let downloads = user_path.join("Downloads");
                    let documents = user_path.join("Documents");
                    for p in [appdata, downloads, documents] {
                        if p.exists() {
                            monitored_paths.push(p);
                        }
                    }
                }
            }
        ...
        // %TEMP% (current process/user temp)
        if let Ok(temp) = std::env::var("TEMP") {
            let tb = PathBuf::from(temp);
            if tb.exists() {
                monitored_paths.push(tb);
            }
        }

        // Initialize known_files by walking the monitored paths (top-level only for speed)
        let mut known_files: HashMap<PathBuf, String> = HashMap::new();
        for base in &monitored_paths {
            if base.is_file() {
                if let Ok(hash) = engine.calculate_sha256(base) {
                    known_files.insert(base.clone(), hash);
                }
                continue;
            }
            if base.is_dir() {
                for entry in WalkDir::new(base).max_depth(1).into_iter().filter_map(Result::ok) {
    ...
    pub fn start(&mut self) {
        ...
        thread::spawn(move || {
            ...
            while !stop_signal.load(Ordering::Relaxed) {
                ...
                // 2. 检查文件新增/修改 — 遍历所有监控路径（仅顶层文件以减少开销）
                for base in &monitored_paths {
                    if base.is_file() {
                        let path = base.clone();
                        if let Ok(hash) = engine.calculate_sha256(&path) {
