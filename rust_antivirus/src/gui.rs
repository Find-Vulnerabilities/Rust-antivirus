...
impl AntivirusApp {
    pub fn new(engine: Arc<AntivirusEngine>, subprocess_manager: SubprocessManager) -> Self {
        let deletion_records = engine.deleted_files.lock().unwrap_or_else(|e| e.into_inner()).clone();
    ...
    fn show_scan_tab(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ui.vertical(|ui| {
            ...
            // 扫描统计
            let scan_count = *self.engine.scan_count.lock().unwrap_or_else(|e| e.into_inner());
            let threats_found = *self.engine.threats_found.lock().unwrap_or_else(|e| e.into_inner());
    ...
    fn start_scan(&mut self, ctx: &egui::Context) {
        ...
        {
            let mut shared = self.scan_results_shared.lock().unwrap_or_else(|e| e.into_inner());
            shared.clear();
        }
        ...
        thread::spawn(move || {
            for path in file_paths {
                // check stop flag
                if *engine.stop_signal.lock().unwrap_or_else(|e| e.into_inner()) {
                    break;
                }
    ...
    fn refresh_deletion_log(&mut self) {
        let records = self.engine.deleted_files.lock().unwrap_or_else(|e| e.into_inner()).clone();
    ...
    fn show_logs_tab(&mut self, ui: &mut egui::Ui) {
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ui.code(&self.log_content);
        });
    }

    fn refresh_logs(&mut self) {
        // 尝试从日志文件读取内容
        // 假设日志文件在当前目录的logs目录或某个位置
        let temp_log = std::env::temp_dir().join("antivirus.log").to_string_lossy().to_string();
        let log_paths = [
            "logs/antivirus.log",
            "antivirus.log",
            &temp_log,
        ...
        // 如果找不到文件，显示默认消息
        self.log_content = "No log files found. Logs may be written to console or other location.".to_string();
    }
}
